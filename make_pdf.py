# -*- coding: utf-8 -*-
"""產生教學檔案處理工具列表的一頁式 PDF（繁體中文）。
使用 Windows 內建 Noto Sans TC 並嵌入 PDF，避免其他閱讀器遺失中文字。
...
if not FONT_PATH.exists():
    raise FileNotFoundError(
        "Missing C:\\Windows\\Fonts\\NotoSansTC-VF.ttf; install Noto Sans TC before rebuilding the PDF."
    )
...
DATA = [
    ("Word", [
        ...
        ("Word 批次轉 PDF/JPG", "Word 檔要逐份另存、再逐頁轉圖",
         "核心: docx2pdf + pypdfium2", "把資料夾的 Word 全部轉成 PDF，再把每頁輸出成 JPG"),
    ]),
    ("Excel", [
        ("成績計算 + 排名 + 及格標紅", "平均、加權、排名、標紅，每次段考重來",
         "核心: openpyxl", "讀 grades.xlsx 算總分與排名，不及格標紅，各班平均放最後一列"),
        ("段考分析（答對率/落點圖）", "想看哪題全班錯最多，但不會做統計圖",
         "核心: matplotlib; 選用: pandas", "分析答題明細，畫各題答對率長條圖和全班分數分布圖"),
        ("總成績單拆成各班/各生", "一張大表要拆給各導師、或每生一張",
         "openpyxl", "把全校成績表依『班級』欄拆成一個班一個 Excel 檔"),
        ("隨機座位表/分組", "每次調座位、分組都要手喬",
         "核心: openpyxl", "用名單隨機排一張 6×5 座位表，輸出成 Excel"),
    ]),
    ("PowerPoint", [
    ...
    ("PDF", [
        ("考卷合併/拆分/重排", "考古題、作業散在幾十個 PDF",
         "pypdf + pypdfium2", "把這些 PDF 合併成一份，並把第 5~8 頁單獨抽出另存"),
        ...
        ("抽課本某幾頁轉圖", "只要課本某張圖貼進學習單",
         "核心: pypdfium2", "把課本 PDF 第 12 頁轉成圖片，去掉白邊"),
    ]),
    ("其他常用", [
        ("連結轉 QR Code", "要學生掃 Padlet/表單，手做 QR 很煩",
         "核心: qrcode[pil]", "把 5 個連結各生一張 QR Code，貼到學習單"),
        ("抓 YouTube 現成字幕做逐字稿", "想要影片逐字稿但不想聽打",
         "選用: youtube-transcript-api", "抓這支 YouTube 影片的字幕，整理成逐字稿"),
        ("講稿轉語音旁白（免費）", "請人配音或買 AI 語音很貴",
         "選用: edge-tts", "把講稿轉成中文語音 mp3，語速慢一點"),
        ("課本轉乾淨文字（餵 AI）", "課本格式亂，丟 AI 效果差",
         "核心: markitdown[文件格式]", "把 PDF/PPT 轉成 Markdown"),
    ]),
]

COLW = [40 * mm, 52 * mm, 42 * mm, 132 * mm]
HEAD = [Paragraph(f"<b>{h}</b>", PHEAD) for h in ("項目", "痛點", "套件", "一句話（複製給 Agent）")]


def build():
    doc = SimpleDocTemplate(
        "教學檔案處理_工具列表.pdf", pagesize=landscape(A4),
    ...
    flow.append(t)
    doc.build(flow)
    print("PDF generated: teacher file tool list")
